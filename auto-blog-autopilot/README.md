...
2. `scripts/generate_post.py`가 `data/topics.txt`의 큐 맨 앞 5개 주제 후보를
   놓고 검색량(Google 트렌드)·자체 유입량/체류시간(GA4)으로 1~5위를 매긴 뒤
...
2. **GA4**로 이 블로그에서 실제로 조회수·체류시간이 좋았던 주제와
   비슷한지

를 함께 점수화해서 1~5위를 매기고, **1위 주제로 그날 글을 씁니다.** 밀린
후보들은 큐 맨 앞쪽에 그대로 남아서 다음날 다시 후보가 됩니다.

**두 가지 모두 선택 사항입니다.** 아무것도 설정하지 않으면 예전과 똑같이
큐 순서대로(FIFO) 발행되고, 원인이 무엇이든(트렌드 조회 실패, GA4 미설정,
데이터 없음 등) 실패하면 자동으로 조용히 FIFO로 넘어갑니다 — 즉 이 설정을
안 해도 기존 기능에는 전혀 영향이 없습니다.
...
#### 7-2. GA4 추적 스크립트 심기 (블로그 방문자 데이터 쌓기 시작)
...
#### 7-3. GA4 데이터를 읽어올 서비스 계정 만들기 (자동화가 읽기 전용으로 조회)

위 7-2는 "쌓기"였고, 이번엔 자동화 스크립트가 그 데이터를 "읽어오기"
위한 인증 정보입니다.
