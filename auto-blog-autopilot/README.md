...
1. 매일 (KST 오전 7시) GitHub Actions가 실행됩니다. 다만 화요일/일요일은
   `REST_WEEKDAYS`로 지정된 휴무일이라, 지정 발행(아래 4번)이 없는 한 스크립트가
   스스로 발행을 건너뜁니다 — 매일 발행 대신 주 5회로 품질에 집중하기 위함입니다.
2. `data/topics.json`에 5개 클러스터(A~E) × 6개, 총 30개 키워드가 고정 풀로
   들어있습니다. 매 실행마다 이 중 하나를 클러스터 가중치 기반 가중 무작위로
   고릅니다:
   ...
   - **D. Remote-Work Tool Troubleshooting** — Zoom/Meet/Drive/Slack/Notion/
     Google Docs 오류 해결 (Troubleshoot 포맷, 경쟁도 낮아 가장 우선 발행)
   - **E. Productivity Software Comparisons** — Notion vs Obsidian 등 정면 비교
     (Comparison 포맷, 광고 친화적이라 우선 발행)

   클러스터별 가중치(`NICHE_CLUSTER_WEIGHT`)는 단순 How-to(A/B)보다 트러블슈팅
   (D)과 비교/대안형(C/E)을 더 자주 고르도록 되어 있습니다 — Google AI
   Overviews가 단순 정보성 How-to 검색의 클릭률을 크게 깎아먹는 반면, 비교·
   대안·구매의도 검색은 상대적으로 안전하다는 신호에 따른 전략입니다. 최근
   사용한 주제(`data/used_topic_ids.json`)는 먼저 제외해서, 풀을 거의 다
   돌기 전까지는 같은 주제가 반복되지 않습니다.
3. 포맷마다 프롬프트 구조가 다릅니다 (`build_how_to_prompt` 등 4개 함수):
   - **How-to**: 직접 답변 → 준비물 → 단계별 설명 → FAQ → 요약
   - **Alternative**: 직접 답변 → 선정 기준 → 목록형 비교 → 표 → FAQ
   - **Troubleshoot**: 직접 답변 → 원인 → 해결단계 → 예방법
   - **Comparison**: 직접 답변(결론 요약) → 비교표 → 항목별 분석 → 추천 대상 → FAQ

   전부 Claude의 `web_search` 도구를 켜서 실제 검색 결과(가격 페이지, 공식
   지원문서 등)를 근거로 쓰게 하고, 그 출처 URL을 `SOURCES` 필드로 받습니다
   (본문에 인용될 뿐 아니라 아래 4번의 스크린샷 캡처 대상으로도 쓰입니다).
   특정 제품에 실제 제휴 링크를 달아 발행하고 싶은 날짜가 있으면(현재는 이
   니치와 맞는 제품이 없어 쓰이지 않지만 기능은 남겨뒀습니다), 이 풀을
   건드리지 않고 "쿠팡파트너스 지정 발행" 오버라이드로 날짜·제품·링크를
   직접 지정할 수 있습니다.
