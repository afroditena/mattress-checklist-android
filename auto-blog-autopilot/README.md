...
2. **평소 요일(월/화/목/금/일)**: `scripts/generate_post.py`가
   `data/topics.txt`의 큐 맨 앞 10개 주제 후보를 놓고 검색량(Google
   트렌드·네이버 데이터랩)·자체 유입량/체류시간(GA4)으로 순위를 매긴 뒤,
   매번 1위만 쓰지 않고 상위 3위 중 하나를 가중 무작위(1위가 더 자주
   뽑힘)로 골라 Claude API로 글을 생성합니다 (설정 안 해도 예전처럼 그냥
   순서대로 FIFO 발행 — 자세한 내용은 "7. 검색량 + 자체 유입량 기반 주제
   순위 매기기" 참고).
   - 이 큐로 나가는 글은 전부 **정보성 글**입니다. Claude의 `web_search`
     도구를 켜서 실제 검색 결과를 확인한 뒤 쓰게 하고, 어떤 제휴/수익화
     링크(쿠팡파트너스 등)도 붙이지 않습니다. 주제는 매트리스/침대/수면환경/
     청소/공기청정기·제습기의 필요성/영양제/건강 관련 정보와, 보험·대출·
     렌탈 같은 재무 관련 정보 위주입니다. 건강·재무처럼 규제가 있는 주제는
     안전장치 프롬프트(단정적 효능·수익 표현 금지, 병원/공식 출처 안내)와
     본문 끝 공식 출처 링크가 자동으로 붙습니다.
   - 특정 제품에 실제 제휴 링크를 달아 발행하고 싶은 날짜가 있으면, 이 큐를
     건드리지 않고 "1. 특정 제품(쿠팡파트너스 링크) 지정 발행" 오버라이드로
     날짜·제품·링크를 직접 지정합니다.
3. **수요일**: 대신 **정치 핫이슈**를 다룹니다 — `web_search`로 미국/유럽/
   아시아/대한민국 중 실제로 화제인 정치 이슈를 찾아서, 편향 없는 사실
   위주 톤으로 씁니다. 정치는 편향 시비 리스크가 커서 요일을 고정해
   정확히 7일에 1일만 나가게 해뒀습니다.
4. **토요일**: **경제/사회/심리/주식** 핫이슈 중 하나를(ISO 주차 기준
   자동 순환) 같은 방식으로 다룹니다. 주식 관련 글에는 투자 권유가
   아니라는 고지와 금감원 DART·한국거래소 같은 공식 출처가 자동으로
   붙습니다.
   - 수요일/토요일 핫이슈 글은 `data/topics.txt` 큐를 건드리지 않아서,
     평소 니치 로테이션에는 영향이 없습니다.
5. 생성된 글을 `docs/_posts/`에 Jekyll 포스트 파일로 저장합니다. 정보성/
   핫이슈 글은 Unsplash 사진을 최대 5장(맨 위 1장 + 소제목마다 1장)
   흩어 넣습니다.
6. 자동으로 커밋·푸시하면, GitHub Pages가 자동으로 사이트를 다시 빌드해서
   글이 공개됩니다.
7. 그날 선택된 주제만 큐 맨 뒤로 돌아가고, 나머지 후보는 그대로 앞쪽에 남아
...
### 4. 쿠팡파트너스로 제휴 수익 추가 (날짜·제품 지정 발행 전용)

`data/topics.txt` 큐로 나가는 정보성 글에는 쿠팡 링크를 전혀 넣지 않습니다.
실제 제휴 링크가 붙는 글은 위 "1. 특정 제품(쿠팡파트너스 링크) 지정 발행"
오버라이드로 날짜·제품·링크를 직접 지정했을 때만 나갑니다:

1. https://partners.coupang.com 가입.
2. 소개하고 싶은 제품을 찾아 "블로그용 태그"로 배너 HTML을 발급.
3. `manual_topic.json`(1건) 또는 `manual_topic_queue.json`(여러 건을
   날짜 순서대로)에 `affiliate_html`/`disclosure_text`로 그대로 넣기.

이 부분은 쿠팡파트너스가 계정별 승인·링크 발급 방식이라 완전 자동화가
...
를 함께 점수화해서 순위를 매기고, **매번 1위만 쓰지 않고 상위 3위 중
하나를 가중 무작위로 골라(1위가 더 자주 뽑힘) 그날 글을 씁니다.** 밀린
...
3. 사용 API에서 **"검색"**과 **"데이터랩(검색어트렌드)"**을 각각
   따로 체크하세요 — 이 둘은 서로 다른 항목입니다. "검색"만 체크하고
   "데이터랩(검색어트렌드)"을 빠뜨리면, Client ID/Secret 자체는 정상
   발급돼도 데이터랩 호출이 401(권한 없음)로 계속 실패합니다.
...
### 8. 보험·금융·렌탈 콘텐츠 (정보 제공, 수수료 없음)

`data/topics.txt`에 보험/금융/렌탈/렌트 관련 주제들을 추가했습니다 (자동차보험료
아끼기, 실손보험 청구, 신용점수 관리, 렌탈 계약 주의사항 등). 이런 주제일 때는
자동으로:

- **AI 프롬프트에 안전장치가 추가됩니다**: 특정 보험료·금리 숫자를 단정적으로
  쓰거나 특정 상품을 추천/가입 유도하지 않고, 일반 원칙과 체크리스트 위주로
  쓰도록 지시합니다.
- **본문 끝에 공식 출처 링크가 자동으로 붙습니다** (금융감독원 파인, 보험다모아,
  한국소비자원 등 — 주제에 맞게). **이건 제휴/수수료 링크가 아니라 순수
  정보 제공용입니다.**
- 관련 없는 쿠팡 쇼핑 링크는 이런 글에는 붙지 않습니다 (진지한 금융 정보
  옆에 뜬금없는 상품 링크가 붙으면 신뢰도만 떨어지니까요).

**왜 지금 수수료 링크를 안 붙이나요?** 보험 상품 가입을 유도하며 수수료를
받는 행위는 한국 보험업법상 등록된 보험설계사·GA(법인보험대리점)만 할 수
있어서, 이 자동화가 임의로 그런 링크를 만들지 않습니다. 실제로 합법적인
제휴 채널(예: 대출·카드 비교 플랫폼의 블로거 파트너스 프로그램)에 가입하시면,
`build_affiliate_block()`과 같은 패턴으로 실제 제휴 링크와 "이 링크로 이용
시 수수료를 받습니다" 안내문을 함께 추가해드릴 수 있습니다. 참고로 살펴볼
만한 채널:

- **신용카드/대출 비교 플랫폼 파트너스** (예: 핀다, 뱅크샐러드 등) — 비교적
  진입장벽이 낮고 제도화된 편입니다.
- **보험 비교·GA사의 블로거 제휴 프로그램** — 보험다모아 자체는 수수료
  프로그램이 없는 공공 비교 사이트이니, 별도로 제휴 프로그램을 운영하는
  GA사를 찾아보셔야 합니다.

각 플랫폼의 "제휴/파트너스" 메뉴에서 실제 가입이 되면, 발급받은 링크 형식을
알려주시면 코드에 반영하겠습니다.
